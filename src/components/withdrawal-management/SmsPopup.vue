...
Popup.h-full.bg-white(:show='show', :safeAreaInsetBottom='true', teleport='body', position='bottom', @update:show='handleUpdateShow')
  ...
  .h-full.grid.grid-rows-content.justify-stretch
    ...
    .px-8.pt-2.pb-12.flex.flex-col.items-stretch
      BasicButton(:loading='postSmsLoading', :disabled='confirmDisabled', bg='accent100', size='large', @click='handleConfirmClick') {{ t("common_confirm") }}
</template>
...
<script setup lang="ts">
  ...
  import { postPhone, postSms } from '@/api/member/member';
  ...
  /**
   * Post sms loading status
   */
  const postSmsLoading = ref(false);
  ...
  const handleConfirmClick = async () => {
    try {
      // Turn loading on
      postSmsLoading.value = true;

      // Post sms
      const response = await postSms({
        code: captcha.value,
        secret: secret.value,
      });
